...
export function MaintenanceDetailsModal({
  ...
  return (
    <Dialog open={!!task} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="w-[90vw] sm:w-full sm:max-w-[500px] max-h-[90vh] flex flex-col p-0">
        <DialogClose asChild>
          <button
            aria-label="Close"
        ...
        <div className="flex-grow overflow-y-auto px-6 py-6 space-y-4 text-sm">
          <div className="grid grid-cols-2 gap-x-4 gap-y-2">
            <strong className="text-gray-500">Status:</strong>
            <span className="capitalize">{task.maintenanceStatus}</span>

            <strong className="text-gray-500">Due Date:</strong>
            <span>{formatDate(task.maintenanceDueDate)}</span>

            <strong className="text-gray-500">Purchase Location:</strong>
            <span>{task.purchaseLocation || "N/A"}</span>

            <strong className="text-gray-500">Cost:</strong>
            <span>${task.costPaid?.toFixed(2) ?? "0.00"}</span>
          </div>
