...
import { Package, Heart, AlertTriangle, Calendar, DollarSign} from "lucide-react";
                  ...
                  {Array.from({ length: 5 }).map((_, index) => (
                    <div key={index} className="flex items-center justify-between p-3 sm:p-4 rounded-lg hover:bg-gray-50">
                      <div className="flex items-center gap-4">
                        <Skeleton className="h-10 w-10 rounded-full" /> {/* Icon placeholder */}
                        <div>
                          <Skeleton className="h-5 w-48" /> {/* Item Title */}
                          <Skeleton className="h-4 w-32 mt-2" /> {/* Item Description */}
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
              ...
              <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md">
                ...
                <div className="space-y-3">
                  {Array.from({ length: 3 }).map((_, index) => (
                    <div key={index} className="flex items-center gap-4 p-3 sm:p-4 rounded-lg hover:bg-gray-50">
                      <Skeleton className="w-12 h-12 rounded-md" /> {/* Image placeholder */}
                      <div>
                        <Skeleton className="h-5 w-32" /> {/* Asset Name */}
                        <Skeleton className="h-4 w-24 mt-2" /> {/* Brand Name */}
                      </div>
                      <Heart className="h-5 w-5 text-gray-300 ml-auto opacity-50" /> {/* Star Icon */}
                    </div>
                  ))}
                </div>
              </div>
