...
export function AddMaintenanceModal({
	...
	useEffect(() => {
		localStorage.setItem(
			"preserveFromPrior",
			JSON.stringify(preserveFromPrior),
		);
	}, [preserveFromPrior]);
	...
	useEffect(() => {
		localStorage.setItem(
			"recurrenceInterval",
			JSON.stringify(recurrenceInterval),
		);
	}, [recurrenceInterval]);
	...
	const addMaintenanceMutation = useMutation({
		mutationFn: async () => {
			if (!user || !selectedAssetId || !selectedAsset)
			...
			const cleanedPayload = Object.fromEntries(
				Object.entries(newMaintenancePayload).filter(
					([_, v]) => v !== "" && v != null,
				),
			);
		...
		onSuccess: () => {
			...
			// Reset form
			if (!assetId) {
				// Don't reset selectedAssetId if it's passed as a prop
				setSelectedAssetId("");
			}
			setTitle("");
			setDueDate("");
		...
		onError: (error) => {
			toast.error(`Failed to add maintenance task: ${error.message}`);
		},
	});
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (toolLocation.length < 2) {
			validationErrors.toolLocation =
				"Tool location must be at least 2 characters.";
		}
		if (!dueDate) {
			...
			<DialogContent className="w-[90vw] sm:w-full sm:max-w-lg max-h-[90vh] flex flex-col p-0">
				...
				<DialogHeader className="-m-[1px] bg-primary-gray text-white px-6 py-4 rounded-t-lg">
					...
					<DialogDescription className="text-white/80 text-center">
						Select an asset and fill in the details for the new maintenance
						task.
					</DialogDescription>
				</DialogHeader>
					...
					>
						...
						{selectedAsset && (
							<div className="p-4 border rounded-md bg-gray-50/80 text-sm">
								<h4 className="font-semibold text-primary-gray mb-2">
									Asset Details
								</h4>
