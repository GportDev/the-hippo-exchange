...
export default function HeaderUser({
	className,
	isNavExpanded,
}: { className?: string; isNavExpanded?: boolean }) {
	const { user } = useUser();
	...
	return (
		...
		>
			...
			<div className="w-16 flex-shrink-0 flex items-center justify-center">
				<div
					ref={userButtonRef}
					className="pointer-events-none scale-140 flex items-center justify-center"
				>
					<UserButton />
				</div>
			</div>
			...
			>
				<div className="whitespace-nowrap overflow-hidden text-ellipsis">
					...
					<p className="text-gray-400 text-sm leading-snug">{user?.username}</p>
				</div>
			</div>
		</button>
	);
